...
export class CarpoolRepository {
  ...
  async deactivateChild(childId: string) {
    ...
    // Delete the child's ride_requests so stale rows don't accumulate.
    // Best-effort: a failure here does not undo the deactivation — the
    // scheduler already filters inactive children (loaded with active=true;
    // childById.get returns undefined for inactive IDs and .filter drops
    // them), so leftover rows are harmless. RLS allows household members
