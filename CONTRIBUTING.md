...
```bash
npm test                 # 120 foundation + scheduling + sites tests (no live DB)
npm run test:integration # 22 tests against live staging Supabase (needs service key in macOS keychain)
npm run test:runtime     # 38 Playwright (8 runtime + 21 E2E + 5 journeys + 4 exploratory)
npx tsc --noEmit
