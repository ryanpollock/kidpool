...
- Crewmate AI is the in-chat coordinator agent (M2, ships disabled): it reads every thread as a visible participant (sender_kind='agent', sender_name='Crewmate AI'), answers schedule questions, and posts structured `chat_proposals` (cancel_ride / switch_slot / swap_drive / coverage_fill) rendered as cards. The LLM never mutates the schedule directly — `confirm_chat_proposal` executes the existing invariant-enforcing RPCs transactionally after the required parent confirms (button tap or detected in-thread consent). Nothing changes unless a parent says OK. **Scope decision (Sep 9, 2026): Crewmate manages custom drives too** — offering, joining, changing, and cancelling custom drives flow through the same consent-gated proposal machinery as regular drives; M2 adds the corresponding proposal kinds and `confirm_chat_proposal` branches calling the custom-drive RPCs.
- Ad hoc custom drives: any parent with a vehicle can offer a one-off drive at an arbitrary time (e.g. a 4:50 PM pickup) and take other kids along. Custom drives are `trips` rows with `slot='custom'`, created mid-week against the **current published schedule_version**, so they appear in every live roster read. Multiple custom drives per day are allowed — the old unique(week_id, service_date, slot) became a partial unique index covering standard slots only. Four security-definer RPCs (202609100002, following the volunteer-RPC invariants — auth/membership/future-trip guards, vehicle auto-selection mirroring `resolveDriverVehicle`, capacity enforcement, audit): `offer_custom_drive(group_id, service_date, direction, meeting_time, child_ids[])` creates the trip + a confirmed driver assignment with the caller's own children as initial riders; `join_custom_drive(trip_id, child_ids[])` is first-come-first-served up to remaining capacity; `leave_custom_drive(trip_id, child_id)`; `cancel_custom_drive(trip_id)` (driver-initiated, returns rider ids for notification). The scheduler EXCLUDES custom drives from optimization and carries them over intact on regeneration (generate-schedule). Reminders include custom drives (cron widened, 202609100003). send-push has `custom_drive_offered` / `custom_drive_joined` / `custom_drive_left` / `custom_drive_cancelled` notification types. Contract tests: `tests/custom-drives.test.mjs`; E2E: `tests/custom-drive.spec.ts`.
- Coordinator oversight: while `groups.coordinator_chat_access` is true, coordinators read and post in ALL group threads (RLS via `can_read_chat_thread`). This near-term flag exists to be flipped off once Crewmate AI has earned trust — coordinators then see only threads they participate in. Do not remove the flag.
- Chat disclosure: every thread creation posts a system note that Crewmate AI will be in the conversation and that nothing changes without a parent's OK. Do not remove the disclosure.
- **Crewmate AI Phase 1 (Q&A-only) is built and ships disabled** (spec: `CREWMATE_REQUIREMENTS.md` at the repo root — read §5/§13 before touching Crewmate). Everything is gated on `groups.crewmate_enabled` (default false, migration `202609140001`): the `notify_chat_agent` trigger (parent messages only) POSTs via pg_net to the `chat-agent` Edge Function (fail-soft vault pattern), which claims a `crewmate_runs` row (`claim_crewmate_run`, one-running-per-thread partial unique index + 90s stale reaping = per-thread coalescing), checks the monthly token budget, triages with a fast model (private `kind='agent'` threads skip triage — always respond), answers via a tool-calling planner (structured read tools + the SELECT-only `crewmate_readonly_query` RPC), and posts ONE agent message. Phase 1 hard boundary: the function's only writes are agent `chat_messages` and `crewmate_runs` — contract-tested (`tests/crewmate.test.mjs`); never add proposal/schedule-table writes to it without updating the requirements doc first. The `crewmate_readonly` role (nologin) + RLS policies scope the SQL tool to one group via the transaction-local `app.crewmate_group` GUC; children is column-granted (no kid phone/photo); profiles/chat tables are not granted at all. The LLM stack: Vercel AI SDK (`ai@7` + `@ai-sdk/openai-compatible` + `zod`) against Together.ai; model IDs are `CREWMATE_TRIAGE_MODEL`/`CREWMATE_PLANNER_MODEL` secrets (defaults Gemma 4 31B IT / DeepSeek V4 Flash), qualified by the eval gate (`npm run eval:crewmate` triage mode; `npm run eval:crewmate:e2e` full staging pipeline) BEFORE a group's flag turns on. Private Crewmate threads: `chat_threads.kind='agent'`, one per parent per group (`ensure_agent_thread` RPC + pinned row in the new-conversation sheet, pinned above Everyone in the inbox). Agent replies push ONLY in agent threads — send-push's `chat_message` branch accepts `sender_kind='agent'` messages there (and only there); everywhere else agent messages arrive silently via Realtime. @Crewmate mentions (202609150001): the one sanctioned null-profile mention form — `{profile_id: null, label: '@Crewmate', span-exact}` — validates in `validate_chat_mentions`; any other null label rejects. The composer pins 'Crewmate AI' in the @-picker for every thread kind except the agent's own private thread. A tagged message is an explicit invocation: chat-agent skips triage AND the NOREPLY second gate (always responds, friendly one-liner for casual tags). Consumers are null-safe by construction (chatRecipients set membership, push title comparisons, span-based <mark> rendering). Crewmate tests: `node --test tests/crewmate.test.mjs` (contracts), the "Crewmate:" block at the end of `tests/integration.test.mjs` (agent threads, claim coalescing, readonly RPC scoping/denylist, flag gate + missing-API-key fail-soft — the function test is local-only), `tests/crewmate.spec.ts` (E2E: pinned entry, disclosure, private-thread invisibility). Enablement runbook (staging first): `docs/crewmate-runbook.md`.
- **Crewmate AI Phase 2 (proposal engine) is built and verified locally, NOT yet on staging** (migrations `202609140002` catalog + `202609140003` proposals/consent/admin tier). The agent now CREATES pending `chat_proposals` cards via a fenced ```crewmate block in the planner reply (validated against a zod catalog; required confirmers resolved from DB facts, never model output); execution stays in Postgres (`execute_chat_proposal` → the catalog RPCs). Dual consent: swap_drive creates two linked cards, first confirm parks, second executes both, and only in threads both drivers can see. In-thread "yes": `confirm_chat_proposal_via_consent` (service-only, evidence-checked, runs the chain with the confirmer's JWT claims). Coordinator `admin_sql`: the DML executor is a definer function OWNED BY the `crewmate_admin` role (confinement by ownership — SET ROLE is illegal in definer frames, and postgres can't grant `auth` schema usage so the actor is passed as a parameter); single statement, table allowlist, group-scoped RLS with-check, RETURNING capture, before/after audit. Contract tests in `tests/crewmate.test.mjs` (Phase 2 block); integration "Crewmate 2:" tests cover range cancels, dual consent, admin confinement + group scoping, consent evidence.
...
- Either-preference children covered on one standard afternoon trip are never double-placed on the sibling (pm_early <-> pm_late): `volunteer_for_uncovered_trip` and `manually_assign_driver` carry the same cross-trip dedup the scheduler, surgical mode, and the client's uncovered alerts have (production incident 2026-09-13 — an Either child seated on 4:20 was re-placed on 5:15 by a pm_late volunteer).
