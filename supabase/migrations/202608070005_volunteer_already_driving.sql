-- Fix volunteer_for_uncovered_trip: handle the case where the caller is
-- already an assigned driver for this trip but their own children aren't
-- in their car.
--
-- Previously the RPC raised 'You are already assigned to drive this trip'
-- which was confusing — the parent sees their child as uncovered, taps
-- 'I can drive', and gets an error. Now the RPC detects the existing
-- assignment and adds the uncovered children to the caller's existing car.
-- If the car is full, it returns a helpful error explaining the situation.
