-- Volunteer for an uncovered trip: when the scheduler left a trip with no
-- driver (or only a tentative/declined driver), an affected parent can
-- volunteer to drive it. Unlike volunteer_for_declined_drive (which takes
-- over an existing declined assignment and moves its rider_assignments),
-- this creates a brand-new confirmed assignment and creates rider_assignments
-- for children who had no tentative/confirmed coverage.
...
begin
  ...
  -- A child is "truly uncovered" for this trip if they have a ride_request
  -- with needs_ride=true AND no rider_assignment to a tentative or
  -- confirmed driver for this trip in this version. Children on a
  -- tentative driver's assignment are not truly uncovered (they have a
  -- pending driver who may still confirm).
  ...
  -- Assign the volunteer's own children first (guaranteed to fit — checked above)
