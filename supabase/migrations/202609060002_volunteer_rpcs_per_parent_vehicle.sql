-- Multi-vehicle support: volunteer RPCs pick the caller's own car, not the
-- household's biggest.
--
-- Households can now have more than one active vehicle, each optionally
-- tagged with its primary driver (vehicles.default_driver_id). When a parent
-- volunteers for a declined or uncovered drive, the RPC must pick THE CAR
-- THEY DRIVE — previously both RPCs picked the household's biggest vehicle,
-- which can overstate a Tesla-driving parent in a minivan household.
--
-- Selection order (mirrors the client-side resolveDriverVehicle helper in
-- carpool-repository.ts):
--   1. The caller's tagged vehicle (default_driver_id = auth.uid()).
--   2. Otherwise the smallest-capacity active household vehicle — never
--      overstates a car. (For a single-vehicle household this is simply
--      that vehicle, so single-car behavior is unchanged.)
-- Note: coalesce(..., false) is required — NULL (untagged) must not sort
-- ahead of a true match under DESC's default NULLS FIRST.
--
-- Bodies are the live staging definitions, which include the uncommitted
-- past-trip guard ('This trip has already happened') on
-- volunteer_for_declined_drive — carrying it to production closes that
-- repo/live gap too (same reconciliation as 202609060001).
